...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const generateDescription = async (index) => {
        ...
        try {
            const { data } = await api.post('api/ai/enhance-job-desc', { userContent: experience.description }, { headers: { Authorization: token } })
    ...
    return (
        <div className='space-y-6'>
            <div className='flex items-center justify-between '>
                ...
                <button onClick={addExperience} className='flex items-center gap-2 px-3 py-1 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-purple-200 transition-colors disabled:opacity-50'>
            ...
            {data.length === 0 ? (
                ...
                <div className='space-y-4'>
                    {data.map((experience, index) => (
                        <div key={index} className='p-4 border-gray-200 rounded-lg spacee-y-3'>
                            ...
                            <div className='grid md:grid-cols-2 gap-3'>
                                ...
                                <input value={experience.end_date && experience.end_date !== "Present" ? experience.end_date : ""} onChange={(e) => updateExperience(index, "end_date", e.target.value)} type="month" disabled={experience.is_current} className='px-3 py-2 text-sm rounded-lg disabled:bg-gray-100' />
                            </div>
                            ...
                            <div className='space-y-2'>
                                <div className='flex items-center justify-between'>
                                    ...
                                    <button onClick={() => generateDescription(index)} disabled={generatingIndex === index || !experience.position || !experience.company || !experience.description} className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded
                                    hover:bg-purple-100 bg-purple-100 text-purple-700
                                    rounded hover:bg-purple-200 transition-colors
                                    disabled:opacity-50'>{generatingIndex === index ? (<Loader2 className='w-3 h-3 animate-spin' />) : (<Sparkles className='w-3 h-3' />)}Enhance with AI</button>
